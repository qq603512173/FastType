...
- **数据**：片段保存在 `%USERPROFILE%\.quicktype\snippets.json`（Windows）或 `~/.quicktype/snippets.json`。

## 功能

- **全局快捷键**：`Alt+Q` 调出/显示窗口。
- **搜索**：在顶部搜索框输入关键字，按标题或内容过滤片段。
- **选择**：方向键或鼠标选择一条结果，回车或双击即可将**内容**粘贴到调出 QuickType 前的焦点位置。
- **托盘**：支持系统托盘，右键可「显示 QuickType」或「退出」。
...
启动后窗口会隐藏，托盘区会出现 QuickType 图标；按 `Alt+Q` 调出搜索窗口。若全局快捷键无效，可尝试**以管理员身份运行**。
...
pyinstaller QuickType.spec
```

生成的 exe 在 **`dist/QuickType.exe`**，可直接发给用户或上传供下载。

- 若项目下有 `build/icon.ico`，打包时会作为 exe 图标；若有 `build/icon.png`，运行时会用作窗口/托盘图标。
- 也可用命令行直接打包：`pyinstaller --onefile --windowed --name QuickType main.py`（不包含 build 图标则需加 `--add-data "build;build"` 才能用图标）。

## 使用说明

1. 运行 QuickType 后，在任意窗口（浏览器、终端、登录框等）按 `Alt+Q` 调出窗口。
2. 在搜索框输入关键字，用 ↑↓ 或鼠标选择一条结果。
3. 回车或双击该条，内容会粘贴到**调出 QuickType 之前**获得焦点的输入位置。
4. 需要新增/修改片段时，点击「编辑片段数据」打开维护界面，新增、编辑或删除后关闭，再次调出 QuickType 会看到更新。
